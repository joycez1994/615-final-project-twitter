...
ggplot(map.data) + 
  geom_map(aes(map_id = region), 
        ...
        ggtitle("Distribution of Coffee Tweets")
```
...
ggplot(combined)+geom_point(aes(x=created,y=screenName,col=screenName))+ggtitle("Span of Tweets Creation")+xlab("Span")+ylab("Brand")
...
```{r}
ggplot(tw.dfs.combined)+geom_point(aes(x=created,y=screenName,col=screenName))+ggtitle("Span of Tweets Creation")+xlab("Span")+ylab("Brand")
...
#For the subset chooses all the tweets in the data(all tweets satisfy sys.time-created<8000), we get the same plot of tweets span.
```
...
```{r warning=FALSE,echo=FALSE}  
plot1<-ggplot(tw.dfs)+geom_point(aes(x=as.Date(created, format = "%m/%d/%y"),y=replyToSN))+ theme(axis.text.y=element_blank())+xlab("Date")+ggtitle("Reply plot of Starbucks")
plot2<-ggplot(tw.dfs.d)+geom_point(aes(x=as.Date(created, format = "%m/%d/%y"),y=replyToSN))+ theme(axis.text.y=element_blank())+xlab("Date")+ggtitle("Reply plot of Dunkin Donuts")
plot3<-ggplot()+geom_point(data=subset(tw.dfs,subset=(!is.na(replyToSN))),aes(x=created,y=replyToSN),col='red') + geom_point(data=subset(tw.dfs,subset=(!is.na(rt))),aes(x=created,y=rt),col='blue') + geom_point(data=subset(tw.dfs,subset=(is.na(replyToSN) & is.na(rt))),aes(x=created,y=screenName),col='green')+ theme(axis.text.y=element_blank())+ggtitle("Reply-Retweet plot of Starbucks")+xlab("Date")
plot4<-ggplot()+geom_point(data=subset(tw.dfs.d,subset=(!is.na(replyToSN))),aes(x=created,y=replyToSN),col='red') + geom_point(data=subset(tw.dfs.d,subset=(!is.na(rt))),aes(x=created,y=rt),col='blue') + geom_point(data=subset(tw.dfs.d,subset=(is.na(replyToSN) & is.na(rt))),aes(x=created,y=screenName),col='green')+ theme(axis.text.y=element_blank())+ggtitle("Reply-Retweet plot of Dunkin Donuts")+xlab("Date")
...
```{r warning=FALSE,echo=FALSE}
plot5<-ggplot(subset(tw.dfs,subset=(!is.na(replyToSN))),aes(x=replyToSN)) + geom_bar(aes(y = (..count..)))+theme(axis.text.x=element_text(angle=-90,size=6))+ theme(axis.text.x=element_blank())+ggtitle("Reply number plot of Starbucks")
head(table(tw.dfs$replyToSN))
plot6<-ggplot(subset(tw.dfs.d,subset=(!is.na(replyToSN))),aes(x=replyToSN)) + geom_bar(aes(y = (..count..)))+theme(axis.text.x=element_text(angle=-90,size=6))+ theme(axis.text.x=element_blank())+ggtitle("Reply number plot of DunkinDonuts")
...
```{r warning=FALSE}
tw.dfs.combined<-rbind(tw.dfs,tw.dfs.d)
ggplot(tw.dfs.combined)+geom_jitter(aes(x=wday,y=hour,color=screenName))+ggtitle("Activeness plot by week and hours")
```
...
```{r warning=FALSE}
ggplot(tw.dfs.combined,aes(x=hour,fill=screenName))  +geom_histogram(aes(y = (..count..)),binwidth=1)+ggtitle("Activeness hours histogram")
```
...
plot7<-ggplot(ts.sum.df)+geom_line(aes(x=date,y=sum))+ggtitle("Tweets timeline of Starbucks")
plot8<-ggplot(ts.sum.df.d)+geom_line(aes(x=date,y=sum))+ggtitle("Tweets timeline of DunkinDonuts")
...
count<-cbind(fc,rtc)
myplot1<-ggplot(data=count,aes(count$fc))+geom_histogram(fill="red")+ggtitle("Favorites Histogram of Starbucks")+xlab("Favorites")
myplot2<-ggplot(data=count,aes(count$rtc))+geom_histogram(fill="blue")+ggtitle("Retweets Histogram of Starbucks")+xlab("Retweets")
...
myplot3<-ggplot(data=count.d,aes(count.d$fc))+geom_histogram(fill="red")+ggtitle("Favorites Histogram of DunkinDonuts")+xlab("Favorites")
myplot4<-ggplot(data=count.d,aes(count.d$rtc))+geom_histogram(fill="blue")+ggtitle("Retweets Histogram of DunkinDonuts")+xlab("Retweets")
grid.arrange(myplot3, myplot4, ncol=2)
```
...
##4.Sentiment analysis of coffee(as whole)
...
barplot(
  ...
  cex.names = 0.7, 
  las = 1, 
  main = "Emotions in Coffee Tweets", xlab="Percentage"
)
